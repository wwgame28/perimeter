...
Открой «📱 Телефон героя», чтобы увидеть внутриигровой интерфейс, или нажми «▶️ Начать вылазку»."""
...
def menu() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="▶️ Начать вылазку"), KeyboardButton(text="📱 Телефон героя")],
        [KeyboardButton(text="🧑‍🚀 Знакомство с героями"), KeyboardButton(text="🗺 Карта")],
        [KeyboardButton(text="🎒 Инвентарь"), KeyboardButton(text="📄 Документы")],
        [KeyboardButton(text="📊 Статус"), KeyboardButton(text="🔄 Сначала")],
    ], resize_keyboard=True)

def phone_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📱 Открыть телефон героя", url=PHONE_URL)],
...
def scene_buttons(scene: str, p: PlayerState) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text="📱 Телефон героя", url=PHONE_URL)])
    if p.maps:
        rows.append([InlineKeyboardButton(text="🗺 Мини-карта", url=map_url(scene, p.maps))])
    return InlineKeyboardMarkup(inline_keyboard=rows)

def team_image():
    if not TEAM_IMAGE_B64.exists(): return None
    raw = TEAM_IMAGE_B64.read_text(encoding="utf-8").strip()
    if not raw or raw == "PLACEHOLDER": return None
    return BufferedInputFile(base64.b64decode(raw), filename="perimeter_team.jpg")

async def send_heroes(obj):
    img = team_image()
    if isinstance(obj, CallbackQuery):
        if img: await obj.message.answer_photo(img, caption="Команда первой вылазки. Главный герой — чёрный силуэт в центре.")
        await obj.message.answer(HERO_INTRO_TEXT)
    else:
        if img: await obj.answer_photo(img, caption="Команда первой вылазки. Главный герой — чёрный силуэт в центре.")
...
async def send_scene(obj, user_id: int, scene_id: str):
    ...
    if isinstance(obj, CallbackQuery): await obj.message.answer(text, reply_markup=scene_buttons(scene_id, p))
    else: await obj.answer(text, reply_markup=scene_buttons(scene_id, p))

async def main():
    if not BOT_TOKEN: raise RuntimeError("Set BOT_TOKEN environment variable")
    bot = Bot(BOT_TOKEN)
    dp = Dispatcher()
    ...
    async def start(m: types.Message):
        ...
        await m.answer("Внутриигровой телефон уже подключён:", reply_markup=phone_keyboard())

    @dp.message(Command("phone"))
    @dp.message(F.text == "📱 Телефон героя")
    async def phone(m: types.Message):
        await m.answer("📱 Телефон героя активен. Через него будут открываться карта, документы, рация, галерея и инвентарь.", reply_markup=phone_keyboard())

    @dp.message(Command("play"))
    @dp.message(F.text == "▶️ Начать вылазку")
    async def play(m: types.Message): await send_scene(m, m.from_user.id, get_player(m.from_user.id).scene)

    @dp.message(Command("heroes"))
    @dp.message(F.text == "🧑‍🚀 Знакомство с героями")
    async def heroes(m: types.Message): await send_heroes(m)

    @dp.message(Command("map"))
    @dp.message(F.text == "🗺 Карта")
    async def show_map(m: types.Message):
        p = get_player(m.from_user.id)
        if not p.maps: await m.answer("🗺 Карта недоступна. Найди схему на локации.")
        else: await m.answer("Открытые карты:\n" + "\n".join(MAP_LABELS[x] for x in p.maps), reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🗺 Открыть мини-карту", url=map_url(p.scene, p.maps))]]))

    @dp.message(Command("inventory"))
    @dp.message(F.text == "🎒 Инвентарь")
    async def inv(m: types.Message):
        p = get_player(m.from_user.id); await m.answer("🎒 Инвентарь:\n" + ("\n".join(ITEM_LABELS[x] for x in p.items) if p.items else "пусто"))

    @dp.message(Command("documents"))
    @dp.message(F.text == "📄 Документы")
    async def docs(m: types.Message):
        p = get_player(m.from_user.id); await m.answer("📄 Документы:\n" + ("\n".join(DOC_LABELS[x] for x in p.docs) if p.docs else "не найдены"))

    @dp.message(Command("status"))
    @dp.message(F.text == "📊 Статус")
    async def status(m: types.Message):
        p = get_player(m.from_user.id); await m.answer(f"📊 Статус\nСцена: {SCENES[p.scene]['title']}\nКарт: {len(p.maps)}/3\nПредметов: {len(p.items)}\nДокументов: {len(p.docs)}")

    @dp.message(Command("restart"))
    @dp.message(F.text == "🔄 Сначала")
    async def restart(m: types.Message):
        reset_player(m.from_user.id); await m.answer("Прогресс сброшен.", reply_markup=menu()); await send_scene(m, m.from_user.id, "start")

    @dp.message(Command("help"))
    async def help_cmd(m: types.Message): await m.answer(HELP_TEXT, reply_markup=menu())

    @dp.callback_query(lambda c: c.data.startswith("go:"))
    async def go(c: CallbackQuery): await c.answer(); await send_scene(c, c.from_user.id, c.data.split(":", 1)[1])

    @dp.message()
    async def fallback(m: types.Message): await m.answer("Используй кнопки меню или /help.", reply_markup=menu())
